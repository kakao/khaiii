...
TEST_F(KhaiiiDevTest, analyze_bfr_errorpatch) {
    ...
    EXPECT_EQ(khaiii::ErrPatch::SENT_DELIM_NUM, static_cast<char32_t>(output[0]));    // bos/eos
    EXPECT_EQ(khaiii::ErrPatch::WORD_DELIM_NUM, static_cast<char32_t>(output[4]));    // bow/eow
    EXPECT_EQ(khaiii::ErrPatch::SENT_DELIM_NUM, static_cast<char32_t>(output[12]));   // bos/eos
